...
# This retriever loads the bundled Paper KG assets and reconstructs document,
# argument/block, example, and reference traversal from JSON plus Chroma indexes.
# Runtime retrieval uses only the visible prompt.
        ...
        deps.append(
            {
                ...
                "evidence_id": f"paper_ref:{resource_type}:{argument_path}:{target}",
            }
        )
...
def _retrieve_paper_faithful_graph_evidence(prompt, kg):
    ...
    if not direct_resources:
        return json.dumps({"source_policy": "No Paper KG evidence found."})
    ...
    for resource_type in all_resources:
        ...
        for block in data.get("required_blocks", []) or []:
            ...
            nested_block_hints.append(
                {
                    "evidence_id": f"paper_block:{resource_type}.{block['name']}",
                    "resource_type": resource_type,
                    "block": block["name"],
                    "required_attrs": [
                    ...
                    "source": "Paper KG Resource-HAS_BLOCK-Argument traversal",
                }
            )
        for block in data.get("optional_blocks", []) or []:
            if block.get("is_identified"):
                ...
                nested_block_hints.append(
                    {
                        "evidence_id": f"paper_identified_block:{resource_type}.{block['name']}",
                        "resource_type": resource_type,
                        "block": block["name"],
                        "required_attrs": [
                            arg["name"] for arg in block.get("required_arguments", []) or []
                        ...
                        "syntax_rule": f"use nested block syntax: {block['name']} {{ ... }}",
                        "source": "Paper KG optional-block selection",
                    }
                )
        candidate_resources.append(
            {
                "type": resource_type,
                "evidence_id": f"paper_resource:{resource_type}",
                "reason": (
        ...
        if example and example.get("code") and MAX_EXAMPLES > 0:
            template_examples.append(
                {
                    "id": f"paper_example:{resource_type}:{example.get('name', '')}",
    ...
    evidence = {
        "source_policy": (
            "Paper KG retrieval over document chunks, argument/block chunks, examples, "
            "and reference traversal. The bundled graph contains benchmark-scoped "
            "relation edges and is reported separately from Full KG. Runtime retrieval uses only the "
            "visible Prompt; no validation, plan, OPA, generated-file, or repair feedback is used."
        ),
        "retrieval_method": {
            "paper_package_mode": "paper_graph",

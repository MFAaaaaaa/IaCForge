...
def load_target_types(dataset_csv):
    resources = set()
    with Path(dataset_csv).open(newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            resources.update(RESOURCE_RE.findall(str(row.get("Resource", "") or "")))
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Disabled by default. Benchmark-scoped KG construction is leakage-prone "
            "and must not be used for generation experiments."
        ),
    )
    parser.add_argument(
        "--allow-dataset-scope-kg",
        action="store_true",
        help="Legacy audit-only escape hatch. Do not use for paper generation experiments.",
    )
    parser.add_argument("--output-dir", default="replication_package/terraform_aws_5.90.0_public_kg")
    args = parser.parse_args()

    if args.dataset_csv and not args.allow_dataset_scope_kg:
        raise SystemExit(
            "--dataset-csv is disabled because it builds a benchmark-scoped KG. "
            "Build the KG from the full public Terraform provider schema/docs instead."
        )
    ...
    if args.dataset_csv:
        target_types = load_target_types(args.dataset_csv)
    else:
    ...
    metadata = {
        ...
        "dataset_csv": str(Path(args.dataset_csv).resolve()) if args.dataset_csv else None,
        "leakage_policy": (
